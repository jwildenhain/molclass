...
function antiInjectie($inputfromform) {

    $search_javascript = array("<script>", "error", "<img", "alert", "<",">");

    $search = array(" ' ", ' " ', " ` ", ' & ', ' , ', ' ; ', '*', '=', 'auth', 'CONNECTION_ID', 'CONCAT', 'CHAR', 'LOAD_FILE', 'SHOW', 'TABLES', 'TABLE', 'SELECT', 'WHERE', 'AND', 'OR','DROP','DELETE','INSERT','CREATE');

    $temp = str_ireplace("'", "`",$inputfromform);
    $temp = str_ireplace("--", "~~",$temp);
    $temp = str_ireplace(";", "~~",$temp);
    $temp = str_ireplace($search, "",$temp);
    $temp = str_ireplace($search_javascript, "",$temp);
