...
if (isset($_GET['compound'])) {
	...
	$mol_id = str_replace("%20"," ",$_GET['compound']);
	/* Fetch Molecule Name */
	$sql = "SELECT mol_id FROM moldb_moldata JOIN inchi_key USING (mol_id) JOIN sdftags USING (mol_id) WHERE mol_id  like '".$mol_id."' or inchi_key.inchi_key like '".$mol_id."' or inchi_key.inchi like '".$mol_id."' or compound_name like '".$mol_id."' or inchi_key.smiles like '".$mol_id."' or mol_name like '".$mol_id."' limit 1";
        //echo $sql;
	$res =& $db->getOne($sql);
	if (PEAR::isError($res))
	{
...
$p->addBodyContent("Sorry this molecule is not in the database.");
}
