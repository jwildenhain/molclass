...
for ($i = 0; $i < $ndb; $i++) {
  ...
  while($str =~ /([^\n]+)\n?/g){
  		...
  		unless($skip) {
             ...
             print($stuff[0]." ".$stuff[1]." ".$stuff[2]." ".$stuff[3]." ".logIt($stuff[2],0.001)."\n");
             #print($stuff[0]." ".$stuff[1]." ".$stuff[2]." ".$stuff[3]." "."\n");
             # update prepare statement
             $stmt = "UPDATE prediction_mols SET mol_id = ?, pred_id =?, main_class =?, distribution =?, lhood =? WHERE  mol_id = ? and pred_id = ?";
