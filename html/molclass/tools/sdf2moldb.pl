...
for ($i = 0; $i <= $#afield; $i++) {
  ...
  $l3  = $afield[$i][2];
	...
	elsif($l2 eq 'plate_number'||$l2 eq 'plate_row'||$l2 eq 'plate_column'||$l2 eq 'plate_row_char'||$l2 eq 'library'||$l2 eq 'sublibrary'||$l2 eq 'chemgrid'){}
	else
...
sub insert_svg() {
  ...
  $newsvg =~ s/\"/\\\"/g;
  #print "$newsvg\n";
  $insertcmd = "UPDATE IGNORE $pic2dtable SET svg = \"${newsvg}\" WHERE mol_id = $entry";
  $dbh->do($insertcmd);
}
